			...
			<div class="modal-body">
			 	<div class="col-md-12">
					<table class="table table-bordered table-striped table-xxs display">
						...
						@php
						    $datas = DB::table('bs_stocks')
						    				->select('stock_masters.brand', 'stock_masters.size', DB::raw("SUM(amount) as amounts"), 'bs_stocks.jan_code')
						    ...
						    // 	$data->stock_out = DB::table('stocks')
						    // 							->select('amount')
						...
						<tbody>
							...
							<tr>
								<td>{{ $data->brand }}</td>
								<td>{{ $data->size }}</td>
								<td class="text-center">{{ $data->amounts }}</td>
							</tr>
							@endforeach
						</tbody>
					</table>
			...
			<div class="modal-body">
			 	<div class="col-md-12">
					<table class="table table-bordered table-striped table-xxs display">
						...
						@php
						    $datas = App\BsStock::select('stock_masters.brand', 'stock_masters.size', DB::raw("SUM(bs_stocks.amount) AS amount"))
						                ->leftJoin('stock_masters', 'stock_masters.jan_code', '=', 'bs_stocks.jan_code')
						                ->whereMonth('receipt_date', $this_month)
						                ->where('client_code', $store->code_from_bs)
						                ->where('titip', 0)
...
<script type="text/javascript">
	...
	$('.btn-type').on('click', function () {
		...
		window.location.href = URL+'new_report_all?type='+type+'&month='+month+'&year='+year;
	});
	...
	function changeFilter(that) {
		type = 'all';
		window.location.href = URL+'new_report_all?type='+type+'&month='+$('[name=month]').val()+'&year='+$('[name=year]').val();
	}
